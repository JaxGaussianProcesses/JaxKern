...
def jax_gather_nd(
    ...
    Args:
        params (Num[Array]): An arbitrary array with leading axes of length `N`
            upon which we shall slice.
        indices (Float[Int]): An integer array of length M with values in the
            range [0, N) whose value at index `i` will be used to slice `params`
            at index `i`.
